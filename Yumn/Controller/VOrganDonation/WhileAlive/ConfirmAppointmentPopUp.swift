...
struct ConfirmAppointmentPopUp: View {
        ...
        addToUserCancellable = Future<Bool, Error> { promise in
            DispatchQueue.main.async {
                ...
                newDoc.setData(["type": appointment.type,"hospital": appointment.hospital, "start_time": appointment.startTime,
                    ...
                    if (error == nil){
                        promise(.success(true))
                        scheduleNotifications(appointment.startTime, appointment.type)
                    } else {
                        print(error!)
                        promise(.failure(error!))
                    }
    ...
    func scheduleNotifications(_ appDate : Date , _ type : String){
        ...
        content.title = "موعد التبرع بالاعضاء"
        content.body = "لديك غداً موعد للتبرع بالاعضاء"
